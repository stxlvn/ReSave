...
def download_and_send_tiktok_photos(task, bot, temp_dir):
    ...
    try:
        ...
        if len(downloaded_files) == 1:
            ...
            file_size_total = sum(os.path.getsize(file_path) for file_path in downloaded_files)
            size_str = format_file_size(file_size_total)
                ...
                with open(file_path, "rb") as file_obj:
                    ...
                    if index == 0:
                        media_group.append(
                            InputMediaPhoto(
                                media=photo_input,
                                caption=MessageTemplate.format_tiktok_photo_caption(
                                    task.url,
            ...
            try:
                ...
                logger.warning(
                    "Ошибка при отправке группы фото, отправляю по отдельности: %s",
                    exc,
                )
                for index, file_path in enumerate(downloaded_files):
                    caption = (
                        MessageTemplate.format_tiktok_photo_caption(task.url)
                        if index == 0
                        else f"🖼️ Фото {index + 1}/{len(downloaded_files)}"
                    )
                    with open(file_path, "rb") as file_obj:
                        bot.send_photo(
                            task.chat_id,
                            file_obj,
                            caption=caption,
                            reply_to_message_id=task.reply_to_id if index == 0 else None,
                            timeout=60,
                            parse_mode="HTML",
                        )
                    time.sleep(0.5)

            file_size_mb = file_size_total / (1024 * 1024)
        ...
        record_download_success(task.chat_id, action="tiktok_photo", file_size_mb=file_size_mb)
    except Exception as exc:
        logger.exception("Ошибка при скачивании TikTok фото: %s", exc)
        if not task.silent_mode:
            from ..utils.message_templates import ErrorMessages
        ...
        for file_path in downloaded_files:
            try:
                ...
                logger.debug("Ошибка при удалении временного файла %s: %s", file_path, exc)
